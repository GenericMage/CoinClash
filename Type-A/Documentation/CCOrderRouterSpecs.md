...
- **Internal Call Flow**:
  ...
  - `_executeSingleOrder`: Constructs `UpdateType` arrays (core, pricing, amounts), calls `ICCListing.ccUpdate`.
...
- **Internal Call Flow**:
  ...
  - `_executeSingleOrder`: Constructs `UpdateType` arrays, calls `ccUpdate`.
