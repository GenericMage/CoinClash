...
- **ICCLiquidityTemplate**: Manages liquidity with `liquidityAmounts` (returns `xAmount`, `yAmount`) and setters (`setRouters`, `setListingId`, `setListingAddress`, `setTokens`, `setAgent`).
- **ITokenRegistry**: Defines `initializeBalances(address token, address[] memory users)` for updating token balances in the registry.
...
- **LastDayFee**: Stores `xFees`, `yFees` (cumulative volumes for fee calculation), and `timestamp` for daily fee tracking, reset at midnight.
...
- `_listingId`: `uint256` - Unique listing identifier, set via `setListingId`.
...
- `_lastDayFee`: `LastDayFee` - Tracks daily fees, updated in `update`, `transactToken`, `transactNative`.
...
- **queryYield(bool isA, uint256 maxIterations)**: Calculates annualized yield (0.05% fee on volume difference), using `maxIterations` to limit historical data traversal. Fetches liquidity from `ICCLiquidityTemplate.liquidityAmounts`, returns 0 if no fees or liquidity. Gas: External call with try-catch, loop up to `maxIterations`.
...
- **update(UpdateType[] memory updates)**: Router-only, updates `_volumeBalance`, buy/sell orders, or `_historicalData`. Updates `_lastDayFee` on volume changes, calls `_updateRegistry`, emits `BalancesUpdated`. Calls `globalizeUpdate` for order globalization if maker exists. Gas: Loop over `updates`, array operations, external calls.
...
- **transactToken(address token, uint256 amount, address recipient)**: Router-only, transfers ERC20 (`_tokenA` or `_tokenB`), updates `_volumeBalance`, `_lastDayFee`, `_currentPrice`, calls `_updateRegistry`, emits `BalancesUpdated`. Gas: External `IERC20.transfer`, array updates.
- **transactNative(uint256 amount, address recipient)**: Router-only, transfers ETH if `_tokenA` or `_tokenB` is `address(0)`, updates `_volumeBalance`, `_lastDayFee`, `_currentPrice`, calls `_updateRegistry`, emits `BalancesUpdated`. Gas: Native call, array updates.

### Internal Functions
- **normalize(uint256, uint8)**: Converts amounts to 1e18 precision. Gas: Pure, minimal arithmetic.
- **denormalize(uint256, uint8)**: Converts amounts from 1e18 to token decimals. Gas: Pure, minimal arithmetic.
- **_isSameDay(uint256, uint256)**: Checks if timestamps are on the same day (86400 seconds). Gas: Pure, arithmetic.
- **_floorToMidnight(uint256)**: Rounds timestamp to midnight. Gas: Pure, arithmetic.
- **_findVolumeChange(bool, uint256, uint256)**: Calculates volume change since `startTime`, using `maxIterations` to limit traversal. Gas: View, loop up to `maxIterations`.
- **_updateRegistry()**: Updates `ITokenRegistry.initializeBalances` for makers in `_pendingBuyOrders` or `_pendingSellOrders`, caps at 500,000 gas, reverts with "Registry call out of gas". Gas: External call, array loops.
- **removePendingOrder(uint256[] storage, uint256)**: Removes order ID from array, swaps with last element and pops. Gas: Array operation.
...
- **Registry** (`_registryAddress`): `ITokenRegistry` address, set via `setRegistry`, used in `_updateRegistry` to initialize maker balances for pending orders.
...
- **ITokenRegistry**: `_updateRegistry` calls `initializeBalances` with makers from `_pendingBuyOrders` or `_pendingSellOrders`, capped at 500,000 gas, reverting with "Registry call out of gas" if exceeded.
- **ICCLiquidityTemplate**: `queryYield` calls `liquidityAmounts` to fetch `xAmount` or `yAmount`, with try-catch to return 0 on failure, ensuring robust yield calculation.
...
- **Fee Tracking**: `_lastDayFee` updates daily (86400 seconds) in `update`, `transactToken`, `transactNative` when volume changes. `queryYield` uses volume difference (`xVolume - xFees`, `yVolume - yFees`) for 0.05% fee calculation.
- **Historical Data**: `update` (type 3) pushes `HistoricalData` with packed `maxPrice` (xBalance/yBalance), `minPrice` (xVolume/yVolume), used in `queryYield` and view functions like `getHistoricalDataView`.
...
- **Security**:
  ...
  - Explicit gas checks in `_updateRegistry` (500,000) and `globalizeUpdate` (gasleft < gasBefore/10) prevent out-of-gas failures, with clear revert messages.
  - Try-catch in `queryYield`, Uniswap V2 calls (`getReserves`), and `globalizeUpdate` ensures graceful degradation.
...
- **Error Handling**: Reverts provide detailed reasons (e.g., "Invalid token", "Insufficient pending", "Router only"). Gas-related reverts ensure robust external call handling.
...
- **_makerPendingOrders**: Despite the name, this mapping stores all order IDs (buy or sell) ever created by a maker, including cancelled (`status=0`), pending (`status=1`), partially filled (`status=2`), or filled (`status=3`) orders. Orders are added in `update` when created (`structId=0`, `status=1`) and removed via `removePendingOrder` when cancelled (`value=0`, `status=0`) or fully filled (`pending=0`, `status=3`). This persistence allows `makerOrdersView` to return historical orders for auditing, while `makerPendingBuyOrdersView` and `makerPendingSellOrdersView` filter for `status=1` to focus on active pending orders. The design balances gas efficiency (single array for all orders) with flexibility for querying both active and historical orders.
